...
def build_preflight_artifacts(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    batches = plan_batches(request_inputs, args.batch_size)
    ...
    first_schema = response_schema(batch_id_for(1), min(args.batch_size, len(request_inputs)))
        ...
        "canary_selected": [
            {
                ...
                "source_global_candidate_id": row["source_global_candidate_id"],
                "strategic_direction_label": row["strategic_direction_label"],
                "canary_profile": row["canary_profile"],
                "canary_selection_reason": row["canary_selection_reason"],
            }
            for row in selected
        ],
        "batch_manifest": [
